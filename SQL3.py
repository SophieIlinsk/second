...
class Connection_Closure:
    def __init__(self):
        self.connect = sqlite3.connect('Bot_Users.db') #создаем табличку
...
class Database(Connection_Closure): #заполнение таблички данными
    def __init__(self):
        super().__init__()
    ...
    def check_user_exists(self, id, user_name): #проверка есть ли пользователь
        self.cursor.execute(
    ...
    def add_user(self, id, user_name): #добавление пользователя
...
class Add_requests(Connection_Closure): #добавление запросов
    def __init__(self):
        super().__init__()
...
# далее много добавления предмета
